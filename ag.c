...
int pertenceA(indvo *ppl,int *v,int k){
	int i,j;
	for(i = 0; i < aux;i++){
		if(ppl->genes_indv[v[i]].dia_sem == k)return 1;
	}
	return 0;
	...
	for(i  = 0; i < qtdsem;i++){
	   ...
	   printf("______________________________________________________________________________________________________________________|\n");
		if(strcmp(sm[i].se,"V1")==0){
			...
			while(1){
				//for(k = 0; k < aux;k++)
					//printf("%d ", ppl->genes_indv[v[k]].dia_sem);
			//	return;
				if(!pertenceA(ppl,v,k) && ispar%2==0){
						printf("  BRANCO      |");
						if(8 == k){printf("\n");printf("10:10-|");j = 0;ispar = 1;k=1;}
											
				}
				else if(!pertenceA(ppl,v,k) && ispar%2!=0){
						printf("      |");						
						if(9 == k){printf("\n");ispar = 2;break;}
					...
					//printf("p:>%d ", ppl->genes_indv[v[j]].dia_sem);
						...
						printf(" %d %s(%s)          |", ppl->genes_indv[v[j]].dia_sem,dsa[ppl->genes_indv[v[j]].disc].cod,ppl->genes_indv[v[j]].prof);
						//printf("t:>%d ", ppl->genes_indv[v[j]].dia_sem);
						if(ppl->genes_indv[v[j]].dia_sem==9){
							ispar = 2;
...
void geraIndividuos(plcao *populacao_t, char *arq){
	...
	while(discomp){
		...
		if(!testaRestricao(ppl->individuos[0], dsa[j], auxsm[numsala].horarios[distt]))continue;
		for(i = 0; i < ppl->individuos[0].qtd;i++){
			...
			strcmp(dsa[j].cod_sem,ppl->individuos[0].genes_indv[i].sem)==0){puts("oi");flag = 1; break;}
		...
		if(!flag){
			...
			//printf(" %d %d \n", dsa[j].periodo,v[j]); 
			if(v[j] == -2){printf("JJJ> %d", j);return;}
			/*if(ppl->individuos[0].qtd>63){
				int p;
				for(p = 0 ; p < qtddisc;p++)
					printf("%d ", v[p]);				
	...
	int b;
	//for(b = 0; b < 62;b++)
	//	if(ppl->individuos[0].genes_indv[b].disc==36)
	//	printf("%d : %d %s \n",b,ppl->individuos[0].genes_indv[b].dia_sem,
	//		ppl->individuos[0].genes_indv[b].prof);
	//return;	
	...
	/*int b = 0;
	for(b = 0;b  < 62;b++)	
		printf("%d %s %s \n", ppl->individuos[0].genes_indv[b].dia_sem,ppl->individuos[0].genes_indv[b].prof,
		ppl->individuos[0].genes_indv[b].sem);
	printf("%d %s %s \n", ppl->individuos[0].genes_indv[1].dia_sem,ppl->individuos[0].genes_indv[1].prof,
	ppl->individuos[0].genes_indv[1].sem);
	printf("%d %s %s \n", ppl->individuos[0].genes_indv[2].dia_sem,ppl->individuos[0].genes_indv[2].prof,
	ppl->individuos[0].genes_indv[2].sem);
	printf("%d %s %s \n", ppl->individuos[0].genes_indv[3].dia_sem,ppl->individuos[0].genes_indv[3].prof
	,ppl->individuos[0].genes_indv[3].sem);
	for(i=0;i<qtdprof;i++) {
		printf("Nome : %s \n",pf[i].nome);
